...
class DemonConvertation(Thread):
    ...
    def folderExist(self):
        if (os.access(self.Excel_folder + '\\' + self.ApprovalProtocolFolder,os.F_OK) == False):
            os.mkdir(self.Excel_folder + '\\' + self.ApprovalProtocolFolder)
        if (os.access(self.Excel_folder + '\\' + self.ApprovalCertFolder,os.F_OK) == False):
            os.mkdir(self.Excel_folder + '\\' + self.ApprovalCertFolder)
        if (os.access(self.Excel_folder + '\\' + self.CalProtocolFolder,os.F_OK) == False):
            os.mkdir(self.Excel_folder + '\\' + self.CalProtocolFolder)
        if (os.access(self.Excel_folder + '\\' + self.CalCertFolder,os.F_OK) == False):
            os.mkdir(self.Excel_folder + '\\' + self.CalCertFolder)
        if (os.access(self.Excel_folder + '\\' + self.ErrorFolder,os.F_OK) == False):
    ...
    def update_settings(self):
        ...
        if self.conf.get('enable','approvalsert') == 'True':
            self.ApprovalSertEnable = True
        else:
            self.ApprovalSertEnable = False
    ...
    def ApprovalReport(self,xml_filename):
        ...
        if len(TestWeightCalibrationAsReturned) > 0:
            ...
            rb = xlrd.open_workbook(self.TemplateApprovalProtocol, formatting_info=True, on_demand=True)  # открываем книгу
            wb = xlcopy(rb)  # копируем книгу в память
            ws = wb.get_sheet(0)  # выбираем лист протокола поверки

            rbCert = xlrd.open_workbook(self.TemplateApprovalCert, formatting_info=True,on_demand=True)  # открываем книгу
            ...
            styleCellLeftSinglCell = xlwt.easyxf('border: top thin, left thin, bottom thin, right thin; align: horiz left')
            ...
            for i in TestWeightCalibrationAsReturned:
                ...
                if TestWeightCalibrations_Count == '1':
                    ...
                    wsCertMert.write(RowMetr, 0, str.strip(Nominal + NominalUnit), styleCellCenterSinglCellTop)
                else:
                    ws.write(Row, 0, str.strip(WeightID + Nominal + NominalUnit), styleCellCenterSinglCellTop)
                    wsCertMert.write(RowMetr, 0, str.strip(WeightID + Nominal + NominalUnit), styleCellCenterSinglCellTop)
            ...
            if Test_Passed == True:
                ...
                wsCert.write(6, 2, CI_Name + " " + TestWeightSet_Range + " " + TestWeightSet_AccuracyClass + " " + TestWeightSet_Comment, styleCellBottom)
                wsCert.write(12,4,TestWeightSet_InternalID,styleCellBottom) # серия и номер клейма предыдущей поверки
                ...
                wsCert.write(20, 2, ReferenceWeightSet_info + ", заводской номер " + ReferenceWeightSet_SerialNumber + ReferenceWeightSet_RegNumber )
                ...
                wsError.write(7, 2, CI_Name + " " + TestWeightSet_Range + " " + TestWeightSet_AccuracyClass + " " + TestWeightSet_Comment, styleCellBottom)
    ...
    def CallReport(self,xml_filename):
        ...
        ReferenceWeightSet_Range = ReferenceWeightSet[0].get('Range') # диапазон набора эталонов
        ReferenceWeightSet_NextCalibrationDate = ReferenceWeightSet[0].get('NextCalibrationDate')  # дата следующей калибровки эталонов
        ReferenceWeightSet_CertificateNumber = ReferenceWeightSet[0].get('CertificateNumber')       # ReferenceWeight_Array = []  # массив наборов эталонов
        ...
        if len(TestWeightCalibrationAsReturned) > 0:
            ...
            wsCertMetr = wbCert.get_sheet(1)  # выбираем лист метрологических характеристик
            ...
            wsCert.write(30, 2, ReferenceWeightSet_info + ", заводской номер " + ReferenceWeightSet_SerialNumber + ReferenceWeightSet_RegNumber, styleCellBottom)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self,parent=None):
        ...
        self.ui.cbxCalSert.setChecked(bool(self.demon.CalSertEnable))
    ...
    def saveSettings(self):
        ...
        self.demon.setAutoStart(self.demon.runing)
    ...
    def selectTemplateApprovalProtocol(self):
        template,ext = QFileDialog.getOpenFileName(self,'Выберите файл шаблона потокола поверки', self.demon.TemplateApprovalProtocol, '*.xls')
        if template != '':
    ...
    def selectTemplateApprovalCert(self):
        template,ext = QFileDialog.getOpenFileName(self,'Выберите файл шаблона свидетельства о поверке', self.demon.TemplateApprovalCert, '*.xls')
        if template != '':
            template = str(template).replace('/','\\')
